...
# inputs = torch.randn(2, 3, 12, 16)
# targets = torch.zeros(2, 12, 16).to(torch.int64)
# targets[:,1,3] = 1
# targets[:,2,3] = 2
# pos_idxs = (targets > 0).nonzero().squeeze(1)
# targets_one_hot = torch.nn.functional.one_hot(targets).to(torch.float32)
# loss = focal_loss.sigmoid_focal_loss(inputs.permute(0, 2, 3, 1), targets_one_hot)
# loss = loss.mean()
...
class ParkModel(BaseModel): 
    ...
    def forward(self, imgs, data_samples=None, mode='tensor'):
        ...
        if mode == 'loss':
            ...
            cls_preds = pld_cls
            pts_preds = torch.cat((pld_pt0, pld_pt1, pld_angle), dim=1)
